...
def test_post_advance_returns_409_when_step_is_not_auto_advance(
    client: TestClient,
):
    client.post("/api/scenarios/test_cafe/new")

    response = client.post("/api/scenarios/test_cafe/advance")
...
def test_post_advance_progresses_step_without_user_dialog(
    ...
    chat_dir.mkdir()
    (chat_dir / "picture.png").write_bytes(b"\x89PNG\r\n\x1a\n")
    chat_graph = {
        "scenario": "test_chat",
        "steps": [
            {
    ...
    _make_step_dir(chat_dir, "1. 인사", script_text="안녕")
    ...
    assert new_response.json()["is_auto_advance"] is True

    response = client.post("/api/scenarios/test_chat/advance")

    assert response.status_code == 200
    body = response.json()
    assert body["current_step_name"] == "2. 끝"
    assert [d["role"] for d in body["dialog"]] == ["character", "character"]


def test_post_advance_returns_404_when_no_progress(client: TestClient):
    response = client.post("/api/scenarios/test_cafe/advance")
    assert response.status_code == 404


def test_post_new_includes_is_auto_advance(client: TestClient):
    response = client.post("/api/scenarios/test_cafe/new")

    body = response.json()
    assert body["is_auto_advance"] is False
