...
def get_step(name: str, step_name: str) -> StepDTO:
    ...
    return StepDTO(
        ...
        script=step.script.read_text(encoding="utf-8"),
...
@router.get("/scenarios/{name}/steps/{step_name}/voice")
def get_step_voice(name: str, step_name: str) -> FileResponse:
    scenario = _require_scenario(name)
    step = _require_step(scenario, step_name)
    return FileResponse(step.voice, headers=_NO_CACHE)
